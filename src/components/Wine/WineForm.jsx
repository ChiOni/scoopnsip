...
export default function WineForm({
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Wine Category */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">와인 분류 *</label>
        <select
          value={wine.wineCategory || 'Red'}
          onChange={(e) => handleChange('wineCategory', e.target.value)}
          className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
          {Object.entries(WINE_CATEGORIES).map(([key, category]) => (
